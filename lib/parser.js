...
function parser(html) {
  ...
  output.suggests = _parseExamples($suggestContainor);
...
function _parseExamples($containor) {
  ...
  $containor.find('.typo-rel').each((index, item) => {
    let $p = $(item);
    let $word = $p.find('span');
    let word = removeTagsAndSpaces($word.html());
    
    // 移除 $word, 防止重复获取 word
    $word.remove();

    let translate = removeTagsAndSpaces($p.html());
    let suggest = new Suggest(word, translate);
